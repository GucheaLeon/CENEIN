...
// ─────────────────────────────────────────────────────────────────────────────
// Subcomponente: Formulario nueva admisión (Etapa 1)
// ─────────────────────────────────────────────────────────────────────────────
function FormularioNuevaAdmision({ onGuardar, onCancelar }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm">
      <div className="w-full max-w-xl rounded-3xl bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div>
            <h2 className="text-lg font-bold text-[#006d44]">Nueva Admisión</h2>
            <p className="mt-0.5 text-xs text-slate-500">Etapa 1 — Datos básicos del paciente</p>
          </div>
          <button
            type="button"
            onClick={onCancelar}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm focus:border-[#006d44] focus:outline-none focus:ring-2 focus:ring-[#006d44]/20"
                placeholder="Ej: Juan"
              />
            </div>
            <div>
          ...
          {/* Fecha / DNI / Teléfono en una fila */}
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label className="mb-1 block text-xs font-semibold text-slate-600 uppercase tracking-wide">DNI</label>
              <input
                id="admision-dni"
                type="text"
                value={form.dni}
            ...
            <div>
              <label className="mb-1 block text-xs font-semibold text-slate-600 uppercase tracking-wide">Teléfono</label>
              <input
                id="admision-telefono"
                type="text"
                value={form.telefono}
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold text-slate-600 uppercase tracking-wide">Domicilio</label>
            <input
              id="admision-domicilio"
              type="text"
          ...
          <div>
            <p className="mb-2 text-xs font-semibold text-slate-600 uppercase tracking-wide">Documentación</p>
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                className={`flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-semibold transition-all ${
                  form.tieneObraSocial
                    ...
                    : 'border-slate-200 bg-slate-50 text-slate-500 hover:border-[#006d44]/40 hover:text-[#006d44]'
                }`}
              >
                <span className="material-symbols-outlined text-base">
                  {form.tieneObraSocial ? 'check_circle' : 'health_and_safety'}
                </span>
                Obra Social
              </button>
              ...
              >
                <span className="material-symbols-outlined text-base">
                  {form.tieneCUD ? 'check_circle' : 'verified'}
                </span>
                CUD
              </button>
            </div>

            {/* Campo nombre OS (si está activo) */}
            {form.tieneObraSocial && (
              <input
                id="admision-os-nombre"
                type="text"
                value={form.obraSocialNombre}
                onChange={(e) => cambiar('obraSocialNombre', e.target.value)}
                placeholder="Nombre de la obra social"
                className="mt-2 w-full rounded-xl border border-[#006d44]/30 bg-[#f0faf4] px-3 py-2 text-sm focus:border-[#006d44] focus:outline-none"
              />
            )}
          </div>
          ...
          <div className="flex justify-end gap-3 pt-1">
            ...
            <button
              type="submit"
              disabled={guardando}
              id="admision-guardar-btn"
              className="rounded-xl bg-[#006d44] px-6 py-2 text-sm font-semibold text-white shadow-sm hover:bg-[#005a38] disabled:opacity-60"
            >
              {guardando ? 'Guardando...' : 'Registrar admisión'}
            </button>
          </div>
        </form>
      </div>
...
function PanelRevisionFisiatra({ admision, onActualizar }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Si ya fue revisado no permitir nueva revisión */}
      {!revision && (
        <>
          {mostrarForm ? (
...
// ─────────────────────────────────────────────────────────────────────────────
// Panel de detalle: expediente / documentos (Etapa 3)
// ─────────────────────────────────────────────────────────────────────────────
const DOCUMENTOS_PDF = [
  { key: 'carnet', field: 'carnetPdf', label: 'Carnet' },
  { key: 'cud', field: 'cudPdf', label: 'CUD' },
  { key: 'consentimiento', field: 'consentimientoPadresPdf', label: 'Consentimiento de los padres' },
  { key: 'presupuesto', field: 'presupuestoPdf', label: 'Presupuesto' },
  { key: 'informe', field: 'informeInicialPdf', label: 'Informe inicial' },
  { key: 'plan', field: 'planTrabajoPdf', label: 'Plan de trabajo' },
  { key: 'historial', field: 'resumenHistorialPdf', label: 'Resumen historial clínico' },
  { key: 'pedidos', field: 'pedidosMedicosPdf', label: 'Pedidos médicos' },
];
...
function PanelExpediente({ admision, onActualizar }) {
  ...
  if (cargando) {
    return <p className="py-4 text-sm text-slate-400">Cargando expediente...</p>;
  }
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-3">
        ...
        {DOCUMENTOS_PDF.map((doc) => {
          const yaSubido = expediente && expediente[doc.field];
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {nuevo ? (
                  ...
                  <a
                    href={urlArchivoExpediente(admision.id, doc.key)}
                    target="_blank"
                    rel="noreferrer"
                    className="text-xs text-[#006d44] underline underline-offset-2 hover:text-[#005a38]"
                  >
                    Ver archivo actual
                  </a>
                ) : (
              ...
              <button
                ...
                className="rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-600 hover:border-[#006d44]/30 hover:text-[#006d44]"
              >
                {yaSubido ? 'Reemplazar' : 'Cargar PDF'}
              </button>
            </div>
          );
        })}
      ...
      {exito && (
        <p className="rounded-xl bg-emerald-50 px-4 py-2.5 text-sm text-emerald-700">{exito}</p>
      )}
      ...
      {admision.estado === 'aprobado' && (
        <div className="flex gap-3">
          <button
            ...
            disabled={guardando}
            className="flex-1 rounded-xl bg-slate-800 px-6 py-3 text-sm font-semibold text-white shadow-sm hover:bg-slate-700 disabled:opacity-60"
          >
          ...
          <button
            type="button"
            onClick={async () => {
              if (window.confirm('¿Estás seguro de finalizar la admisión? Se marcará el expediente como completo.')) {
                setGuardando(true);
                try {
                  ...
                  if (Object.keys(archivos).length > 0 ||
                    ...
                    await handleGuardar(new Event('submit'));
                  }
            ...
            disabled={guardando}
            className="flex-1 rounded-xl bg-[#006d44] px-6 py-3 text-sm font-semibold text-white shadow-sm hover:bg-[#005a38] disabled:opacity-60"
          >
            Finalizar admisión
          </button>
        </div>
      ...
      {admision.estado === 'completado' && (
        <div className="flex gap-3">
          <button
            ...
            onClick={handleGuardar}
            disabled={guardando}
            className="w-full rounded-xl bg-slate-800 px-6 py-3 text-sm font-semibold text-white shadow-sm hover:bg-slate-700 disabled:opacity-60"
          >
            {guardando ? 'Guardando...' : 'Actualizar documentos'}
          </button>
        </div>
      )}
    </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// Panel lateral de detalle de admisión
// ─────────────────────────────────────────────────────────────────────────────
function PanelDetalle({ admision, onCerrar, onActualizar }) {
  const [pestana, setPestana] = useState('datos');
  // Estado local para OS/CUD editables
  ...
  // Cargar obras sociales cuando se activa OS
  React.useEffect(() => {
  ...
  // Sincronizar si cambia la admision desde afuera
  React.useEffect(() => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-semibold transition ${
                  isSelected ? 'bg-[#006d44] text-white' : unselectedStyle
                }`}
              >
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5">
        {pestana === 'datos' && (
          <div className="space-y-3">
            ...
            {/* Tarjetas interactivas OS / CUD — lado a lado */}
      ...
      <div className="border-b border-slate-200 bg-white px-6 py-4 sm:px-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            ...
            type="button"
            onClick={() => setMostrarForm(true)}
            className="flex items-center gap-2 self-start rounded-2xl bg-[#006d44] px-5 py-2.5 text-sm font-semibold text-white shadow-md hover:bg-[#005a38] sm:self-auto"
          >
                  ...
                  <li key={admision.id}>
                    <button
                      ...
                      className={`group w-full px-5 py-4 text-left transition hover:bg-slate-50 ${
                        seleccionada?.id === admision.id ? 'bg-[#f0faf4]' : ''
                      }`}
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                            ...
                            {(admision.estado === 'aprobado' || admision.estado === 'completado') && (
                              <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-0.5 text-xs font-medium text-emerald-600">
                                <span className="material-symbols-outlined text-xs">check_circle</span>
                                Revisión fisiátrica aprobada
                              </span>
                            )}
                          </div>
                        </div>
